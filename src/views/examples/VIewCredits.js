  ...
  useEffect(() => {
    const getAllCredits = async () => {
      try {
        ...
        const { data: response } = await axios.get(`${baseUrl}/api/customer/getCredits/${customerId}`);
        setCreditData(response.data.data);
        setTotalCreditAmount(response.data.totalCreditAmount);
        // console.log(response.data.totalCreditAmount);
      } catch (error) {
        console.log(error);
      }
          ...
          <div className="col">
            <Card className="shadow">
              <CardHeader className="border-0">
                <h3 className="mb-0">{customerName}  Balance: {totalCreditAmount}</h3>
              </CardHeader>
                    ...
                    <tr key={index}>
                      <th scope="row">
                        <Media className="align-items-center">
                          <Media>
                            <span className="mb-0 text-sm">{formatDate(credit.createdAt)}</span>
                          </Media>
                        </Media>
                      </th>
                      <td>₹{credit.amount}</td>
                      <td>
                        <Badge color="" className="badge-dot mr-4">
                          <i className={`bg-${credit.status === "Credit Paid" ? "success" : "warning"}`} />
                          {credit.status !== "Credit Paid" ? "Credit" : "Paid"}
                        </Badge>
                      </td>
                      <td>{credit.adminId?.fullName}</td>
                    </tr>
                  ))}
